(function() {
  ...
  vexDialogFactory = function($, vex) {
    ...
    dialog.buildDialogForm = function(options) {
      var $title, $form, $input, $message, $ajaxHtml, content;
      $title = $('<p class="vex-title"></p>');
      $form = $('<form class="vex-dialog-form" />');
      $message = $('<div class="vex-dialog-message" />');
      $input = $('<div class="vex-dialog-input" />');
      if(options.ajaxHtml != ""){
        $ajaxHtml = $('<div class="vex-dialog-ajax" />');
        content = $ajaxHtml.load(options.ajaxHtml);
      }      
      $form.append($title.append(options.title)).append($message.append(options.message)).append($input.append(options.input)).append(content).append(dialog.buttonsToDOM(options.buttons)).bind('submit.vex', options.onSubmit);
      return $form;
    };
