        ...
        funUploadFile : function(file){
            ...
            xhr.addEventListener("load", function(e){
                ...
                if(xhr.status===200){
                    ...
                    self.onFailure(file, xhr.responseXML);
                }
            ...
            if (Uploadinfo == "") {
                ...
                for(var field_name in new_multipart_params){
                    formdata.append(field_name,new_multipart_params[field_name]);
                }
    ...
    $.fn.zyUpload = function(options,param){
        ...
        return this.each(function(){
            ...
            this.createHtml = function(){
                ...
                $(".upload_onit").css("width",para.width);
            ...
            this.funSetStatusInfo = function(files){
                ...
                // 设置内容
                $("#status_info").html("选中"+num+"张文件未上传，共"+size+"。");
            };
            ...
            this.funFilterEligibleFile = function(files){
                ...
                for (var i = 0, file; file = files[i]; i++) {
                    if (para.multiple == false && $(".imgBox").length == 1) {
                        alert("当前只能上传一个文件");
                    }else if ($(".imgBox").length > 9) {
                        alert("最多只能上传10张图片");
                    }else if (file.size >= 4194304) {
                        alert('您这个"'+ file.name +'"文件大小过大，不能大于4M');
                    }else if(file.type != "image/jpeg" && file.type != "image/png" && file.type != "image/bmp" && file.type != "image/gif") {
                        alert('您这个"'+ file.name +'"格式必须是图片格式文件');
                    }else if(file.name.length >= 50) {
                        alert('您上传的"'+ file.name +'"图片文件名太长');
                    } else {
                        // 在这里需要判断当前所有文件中
                        arrFiles.push(file);
                    }
            ...
            this.createCorePlug = function(){
                var params = {
                    ...
                    onFailure: function(file) {
                        $("#uploadProgress_" + file.index).hide();
                        //$("#uploadSuccess_" + file.index).show();
                        swal("出问题了","文件" + file.name + "上传失败！","error");
                        $("#uploadInf").append("<p class='tmpuploadinfo' style='color:red'>文件" + file.name + "上传失败！</p>");
                        $(".upload_onit").hide();
                        // YouTravel.UI.removecornerload();
                        //$("#uploadImage_" + file.index).css("opacity", 0.2);
                    },
                    onComplete: function(response){
                        console.info(response);
                        // YouTravel.UI.removecornerload();
                        //$("#status_info").html("选中0张文件未上传，共0B。");
                        $(".upload_onit").hide();
                        $(".btn-default").removeAttr("disabled");
                        $(".close").removeAttr("disabled");
                        $(".addLinesubmit").removeAttr("disabled");
                        // 重新设置统计栏信息
                        self.funSetStatusInfo(ZYFILE.funReturnNeedFiles());
                    },
                    onDragOver: function() {
            ...
            this.addEvent = function(){
                ...
                if($(".filePicker").length > 0){
                    // 绑定选择事件
                    $(".filePicker").unbind().bind("click", function(e){
                        $("#fileImage").click();
                    });
                }

                // 绑定继续添加点击事件
                $(".webuploader_pick").unbind().bind("click", function(e){
                    $("#fileImage").click();
                });

                // 绑定上传点击事件
                $(".upload_btn").unbind().bind("click", function(e){
                    // 判断当前是否有文件需要上传
                    if(ZYFILE.funReturnNeedFiles().length > 0){
                        if ($(".imgBox").length > 10) {
                            ...
                            $(".addLinesubmit").attr("disabled","disabled");
                            $(".tmpuploadinfo").remove();
                        }
                    }else{
                        alert("请先选中文件再点击上传");
                    }
                ...
                if($("#rapidAddImg").length > 0){
                    // 绑定添加点击事件
                    $("#rapidAddImg").unbind().bind("click", function(e){
                        $("#fileImage").click();
                    });
                }
            };
